...
refs.button.textContent = 'Добавить логин';
...
const isLoginUnique = function(allLogins, login) {
    allLogins = allLogins.join(' ').toLowerCase().split(' ');
    login = login.toLowerCase();
    for (let index of allLogins){
    if (index == login){
        return false;
        };
    }
...
const addLogin = function(allLogins, login) {
  if (isLoginValid(login)) {
    if (isLoginUnique(allLogins, login)){
        ...
        return alert('Логин добавлен');
    };
        return alert('Такой логин уже существует');
  };
    return alert('Логин должен состоять от 4 до 16 символов');
};

refs.button.addEventListener('click', function(){
    addLogin(logins,refs.input.value);
});